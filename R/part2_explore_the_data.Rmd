...
```{r, echo=F}
my_cols = cols(condition='f', subject='f', trial='f', score='i', ignap='d', ignba='d', icint='d')
ini <- read_csv("d3_trial_by_trial.csv", col_types=my_cols)
```
...
pdata = ini %>% 
  ...
  mutate(fruit=ifelse(fruit=="ignap","apples","bananas"))
ggplot(pdata, aes(x=subject, y=rejection_rate)) +
  theme_bw()+
  facet_wrap(~condition, scales='free_x')+
  stat_summary(geom='pointrange',fun.data='mean_se', aes(color=fruit))+
  stat_summary(geom='point',fun = 'mean', pch=21, aes(color=fruit), size=5)+
  labs(subtitle='mean rejection rates per subject')+
...
tbt <- read_csv("d3_trial_by_trial.csv", col_types=my_cols) %>% 
...
ggplot(freqhi %>% mutate(subject=as_factor(subject)), aes(x=rejection_rate_for_apples, y=resid)) +
  facet_grid(~condition) +
  geom_point(aes(group=subject, color=subject, pch=subject)) +
  geom_smooth(se=F,method='lm',aes(group=subject, color=subject))+
  labs(subtitle='line per subject') + 
  ylab("score (residualised)") +
  theme_bw() +
...
```{r}
ggscatter(freqhi %>% filter(subject!=3) %>% mutate(subject=as_factor(subject)), x="rejection_rate_for_apples", y="resid", add='reg.line', conf.int=F, add.params = list(color = "subject", fill = "lightgray"), facet.by='condition') + 
  # stat_cor(method='pearson')+
  theme(legend.position = 'none') + 
  labs(subtitle='line per subject') + 
  ylab("score (residualised)") +
  labs(subtitle = 'after removing subject 3')
#+
  # geom_ref_line(h=0,colour='grey')
...
```{r}
# add intercept and slope to freqhi for an lem fitted to each subject
by_subject <- freqhi %>% 
  group_by(subject,condition) %>% 
  nest()
subject_model <- function(df){
  lm(resid ~ rejection_rate_for_apples, data=df)
}
