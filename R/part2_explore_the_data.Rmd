...
```{r, 'Rejection rate against score', fig.width=7, fig.height=4}
ggplot(tbt, aes(y=rejection_rate_for_apples, x=score)) + scale_color_manual(values=c("blue","red")) +
  geom_jitter(width=.5,height=0.005,alpha=.5) + 
  geom_smooth(method='lm', aes(color=condition)) +theme(aspect.ratio = 1)+ facet_wrap(~condition)+theme(legend.position="top")
```
