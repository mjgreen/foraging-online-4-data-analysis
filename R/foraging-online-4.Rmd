...
get <- function(){
  ...
  for (fl in fls) {
    ...
    this_file <- fin %>% 
      ...
      rename(npoints=points_list) %>% 
      select(date, participant, condition, phase, trial, times, frames, events, trees, npoints)
  ...
  for(p in levels(dat$participant)) {
    for (t in unique(dat$trial)) {
      ...
      temp = tibble(
        ...
        npoints=d$points_list %>% str_sub(start=2,end=-2) %>% str_split(pattern=',') %>% unlist() %>% as_tibble_col() %>% pull() %>% as.numeric(),
        points_inc = npoints>lag(npoints,default=FALSE)
      )
...
e <- e %>% filter(!(event==lead(event)&tree==lead(tree)&npoints==lead(npoints)&trial==lead(trial)))
